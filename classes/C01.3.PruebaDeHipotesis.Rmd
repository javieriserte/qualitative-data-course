...
```{r}
t_left <- qt(0.05/2, df = n-1)
t_right <- qt(1-0.05/2, df = n-1)
...
Ei <- 1000 * pbinom(q=xs, size = 100, p=0.055)
...
my_other_sample <- runif(10000, 0, 4)
...
El test de Student asume que las varianzas son iguales, pero existe una alternativa a este test, tste de Welch, que no requiere este asumpción.

```{r}
sample_1 <- rnorm(15, 2, 1)
sample_2 <- rnorm(15, 3, 1)
...
Se usa si se tienen dos variables nominales, y se quiere saber si las proporciones de una variable se mantiene entre los valores de la otra variable.
...
```{r}
sample_1 <- rbinom(25, 15, 0.04 )
sample_2 <- sample_1 + runif(25, 2, 4 )
mcnemar.test(
    table(
        c(sample_1, sample_2)>=m,
        rep(1:2, each=25))
